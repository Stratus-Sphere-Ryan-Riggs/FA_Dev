...
function updatePurchaseOrderFromSalesOrder(type) {
    try {
      ...
      //Added by Thilaga for blue donations
      if(thisSalesOrder.getFieldValue('custbody_order_type')=='6'){
        return true;
      }
        ...
        var oldStatus = oldSalesOrder.getFieldValue('custbody_order_status');
        var status = thisSalesOrder.getFieldValue('custbody_order_status');
      	var oldOrderStatus = oldSalesOrder.getFieldValue('status');
      	var newOrderStatus = thisSalesOrder.getFieldValue('status');
      nlapiLogExecution('debug', 'oldStatus : new status' + oldStatus+" : "+status+" Cancelled:"+cancelled+" Closed:"+closed);
      nlapiLogExecution('debug', 'oldOrderStatus : newOrderStatus' + oldOrderStatus+" : "+newOrderStatus);
        //cancelled = 'C';
       // closed = 'H';

        if (((oldStatus != status) && (status == cancelled)) || ((oldOrderStatus!=newOrderStatus) && (newOrderStatus == closed)) &&(!isEmpty(thisSalesOrder.getFieldValue('custbody_associated_transaction')))) {
        ...
        var donationProgram = nlapiGetContext().getSetting('SCRIPT', 'custscript_donation_program2');
        ...
        if(thisOrderType == yellow || thisOrderType == maroon){
        ...
        var poBillingStatus = purchaseOrder.getFieldValue('statusRef');
        nlapiLogExecution('DEBUG', 'poBillingStatus' + poBillingStatus);
        ...
        if (poBillingStatus != 'fullyBilled') {
        ...
        var transportVariance = nlapiGetContext().getSetting('SCRIPT', 'custscript_transport_variance');
      	var toleranceLow=parseFloat(0)-parseFloat(nlapiGetContext().getSetting('SCRIPT', 'custscript_var_threshold_low'));
      ...
      nlapiLogExecution('debug', ' freightCostDiff : ' + tolerance);
        nlapiLogExecution('debug', ' transportVariance : ' + toleranceLow);
 //commented by Thilaga for ticket #4804
 ////Added by Thilaga for ticket #6580
 //Added by Elizabeth - No change to approval status is PO is already approved for ticket #7317
 		
        if (freightCostDiff >= tolerance || freightCostDiff<=toleranceLow) {
          var poStatus = purchaseOrder.getFieldValue('approvalstatus');
          if (poStatus != '2') {
            purchaseOrder.setFieldValue('approvalstatus', defApproval);
        nlapiLogExecution('DEBUG', 'here1' + defApproval);
          }
        ...
        if (thisOrderType == produce) {
            if (poBillingStatus != 'fullyBilled') {
            var prItem = nlapiGetContext().getSetting('SCRIPT', 'custscript_produce_variance_item');
            description = "Transportation variance cost for Sales Order : " + thisSalesOrder.getFieldValue('tranid');
            var foundItem = purchaseOrder.findLineItemValue('item', 'item', prItem);
            if (foundItem != '-1') {
                purchaseOrder.setLineItemValue('item', 'rate', foundItem, freightCostDiff);
                purchaseOrder.setLineItemValue('item', 'amount', foundItem, freightCostDiff);
              newPosition++;
                ...
                setLine(purchaseOrder, prItem, 1, date, description, freightCostDiff, freightCostDiff, newPosition, program);
                newPosition++;
            }
          }
        }
        ...
        if (thisOrderType == disaster) {
            if (poBillingStatus != 'fullyBilled') {
            var tranItemDesc = "Transportation estimated for Sales Order : " + thisSalesOrder.getFieldValue('tranid');
            var foundFirstItem = purchaseOrder.findLineItemValue('item', 'item', itemToInsert);
            if (foundFirstItem != '-1') {
                purchaseOrder.setLineItemValue('item', 'rate', foundFirstItem, 0);
                purchaseOrder.setLineItemValue('item', 'amount', foundFirstItem, 0);
            }
            var disItem = nlapiGetContext().getSetting('SCRIPT', 'custscript_disaster_tran_item');
            description = "Transportation est cost for Sales Order : " + thisSalesOrder.getFieldValue('tranid');
            var foundItem = purchaseOrder.findLineItemValue('item', 'item', disItem);
            if (foundItem != '-1') {
            nlapiLogExecution('DEBUG', 'foundItem' + foundItem);
                purchaseOrder.setLineItemValue('item', 'rate', foundItem, actualFreightCost);
                purchaseOrder.setLineItemValue('item', 'amount', foundItem, actualFreightCost);
              newPosition++;
            }
            else {
                setLine(purchaseOrder, disItem, 1, date, description, actualFreightCost, actualFreightCost, newPosition, program);
                newPosition++;
            }
            }
        }

      if (thisOrderType == yellow || thisOrderType == maroon || thisOrderType == seafood) {
            if (poBillingStatus != 'fullyBilled') {
            var tranItemDesc = "Transportation estimated for Sales Order : " + thisSalesOrder.getFieldValue('tranid');
            var foundItem = purchaseOrder.findLineItemValue('item', 'item', itemToInsert);
            ...
            nlapiLogExecution('DEBUG', 'foundItem' + foundItem);
              ...
              newPosition++;
                ...
                setLine(purchaseOrder, itemToInsert, 1, date, description, actualFreightCost, actualFreightCost, newPosition, program);
                newPosition++;
            }
            }
        }
