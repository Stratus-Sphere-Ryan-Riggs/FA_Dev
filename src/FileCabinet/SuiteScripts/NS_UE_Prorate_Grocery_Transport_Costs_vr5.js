...
nlapiLogExecution('DEBUG','Accessorial Fee',accessorialFee);
	...
	if ((groceryType == recTrans.getFieldValue('custbody_order_type')) && (recTrans.getFieldValue('custbody_associated_salesorder')!=null) && (recTrans.getFieldValue('custbody_associated_salesorder')!=''))
	{
        ...
        {
            //added by Thilaga for ticket #6523
            if(recTrans.getLineItemValue('item','rate',j)=='0.00' && (recTrans.getFieldValue('entity')!=recTrans.getLineItemValue('item','custcol_member_bank',j))){
                continue;
            }
			...
			if (itemChannel == groceryChannel || (recTrans.getLineItemValue('item','item',j)==secGroItem))
			{
              nlapiLogExecution('DEBUG','Inside secondary grocery item','Qty');
				ttlQuantity = ttlQuantity + parseFloat(recTrans.getLineItemValue('item','custcol_original_order_qty',j));
			}	
	    ...
	    if (ttlQuantity > 0 && totalTransportCost > 0)
		{
	 	     ...
	 	     for (var i = 1; i <= recTrans.getLineItemCount('item'); i++ ) 
             {
                 ...
                 if(recTrans.getLineItemValue('item','rate',i)=='0.00'){
		           ...
		           {
                     nlapiLogExecution('DEBUG','Inside secondary grocery item','set prorated value '+recTrans.getLineItemValue('item','item',i)+" tscostperitem:"+tsCostPerItem);
...
recTrans.setFieldValue('custbody_ttl_transporation_fees',nlapiFormatCurrency(transportTotal));
      ...
      if(recTrans.getFieldValue('custbody_ttl_transporation_fees')>0 && (parseFloat(nlapiGetFieldValue('amount'))!=recTrans.getFieldValue('custbody_ttl_transporation_fees'))){
