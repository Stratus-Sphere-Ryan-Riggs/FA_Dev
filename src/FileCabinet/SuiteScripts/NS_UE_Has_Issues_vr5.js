...
function beforesubmit_checkhasissues(stType)
{
  ...
  //Added for ticket#7093 by Thilaga
  if(recType=='vendorbill' && nlapiGetFieldText('custbody_vendor_bill_type')=='Admin Fee'){
  		return;
	}
...
function aftersubmit_addreleasenumber(stType)
{
  ...
  //Added for Bug #6599
	...
	if ((stType.toString() === 'dropship' || stType.toString() === 'create') && transType === 'purchaseorder') {
        poRecord = nlapiLoadRecord('purchaseorder', orderID);
        poOrderType = poRecord.getFieldValue('custbody_order_type');
        nlapiLogExecution('DEBUG', 'custbody_order_type ', ' Type= ' + poOrderType);
        if (poOrderType === '1' || poOrderType === '2') {
            nlapiLogExecution('DEBUG', 'entered orderType ', ' Type= ' + poOrderType);
            createdFrom = poRecord.getFieldValue('createdfrom');
            nlapiLogExecution('DEBUG', 'createdFrom', createdFrom);
          	if(createdFrom != null || createdFrom != ''){
              if(poRecord.getFieldValue('custbody_vendor_bill_type') != '1'){
            	poRecord.setFieldValue('custbody_associated_salesorder', createdFrom);
            	poRecord.setFieldValue('approvalstatus', '2');
    			poRecord.setFieldValue('custbody_po_status', '2');
