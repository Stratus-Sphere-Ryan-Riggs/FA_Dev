  ...
  if(transtype == 'itemfulfillment'){
    ...
    for (var i = 1; i <= nlapiGetLineItemCount('item'); i++ ) 
    {
      ...
      //Commented for ticket #5151 as rounding is not expected to happen
			...
			//Added for PO Admin ticket
         	nlapiLogExecution('DEBUG','beforeload_load>>orderType',orderType);
         	if(orderType=='2'){
              salesRec = nlapiLoadRecord('salesorder',soid);
              var magRef = nlapiGetLineItemValue('item','custcol_mag_ref_no',i);
              var lineId = salesRec.findLineItemValue('item','custcol_mag_ref_no',magRef);
              var rate = parseFloat('0');
              var poRate = parseFloat('0');
              var adminFeePerQty = nlapiGetLineItemValue('item','custcol_produce_adm_fee_qty',i);;
             
              nlapiLogExecution('DEBUG','beforeload_load>>lineId',lineId);
              if(lineId!='-1' && (adminFeePerQty==null || adminFeePerQty=='')){
                rate = salesRec.getLineItemValue('item','rate',lineId);
                poRate = salesRec.getLineItemValue('item','porate',lineId);
                adminFeePerQty = parseFloat(rate)-parseFloat(poRate);
                nlapiSetLineItemValue('item', 'custcol_produce_adm_fee_qty', i, parseFloat(adminFeePerQty));
                nlapiLogExecution('DEBUG','beforeload_load>>adminFeePerQty>>rate>>poRate',adminFeePerQty+' '+rate+' '+poRate);
              }
            }
...
function aftersubmit_setpounds(stType)
{
  ...
  //Adjusted by Thilaga for ticket #Bug 5018
	...
	if (transtype == 'itemfulfillment' && (!Eval.isEmpty(recTrans.getFieldValue('createdfrom'))) )
	{
      ...
      if (icSearchResults_2 != null && icSearchResults_2.length != 0) 
      {
			...
			for (var i = 0; i < icSearchResults_2.length; i++)
        	{
              ...
              nlapiLogExecution('EMERGENCY','item','|------------SCRIPT STARTED------------||'+item);
              ...
              var ttlQty = 
                  parseFloat(icSearchResults_2[i].getValue('quantity', null, 'sum')); 
                nlapiLogExecution('EMERGENCY','received Qty for item','|------------SCRIPT STARTED------------|| SOID'+ttlQty+' '+soId);
              ...
              if(item!='4148'){
                ...
                nlapiLogExecution('Debug', 'lineep ' + lineNum + ' Pounds ' + ttlPounds + ' Pallet ' + ttlPallets);
                nlapiLogExecution('Debug', 'lineep found ' + lineid);
              }
              nlapiLogExecution('EMERGENCY','total received Qty for item','|------------SCRIPT STARTED------------|| SOID'+ttlReceivedQty+' '+soId);
		    ...
		    for (var j = 1; j <= soTrans.getLineItemCount('item'); j++ ) 
            {
              ...
              var itemGrossWeight = soTrans.getLineItemValue('item','custcol_item_gross_weight',j);
				...
				if (qtyFulfill > 0||(soTrans.getLineItemValue('item','custcol_item_status_field',j)==line_status)||
                    (soTrans.getLineItemValue('item','quantity',j)==0))
				{
             		nbrReceiptedLines++
                    if(qtyFulfill > 0 || (soTrans.getLineItemValue('item','quantity',j)==0) ){
				...
				//Added for ticket #7216 by Thilaga
				if(qtyFulfill == 0){
                  soTrans.setLineItemValue('item','custcol_gross_wt_received',j,'0');
                }
        ...
        //Added by Elizabeth for ticket #8010
        
        if (orderType == '3' || orderType == '5' || orderType == '6' || orderType == '9' || orderType == '8' || orderType == '2') {
            var donorID = nlapiLookupField('salesorder',soId,'custbody_opportunity_donor');
            var pickup_date = nlapiLookupField('salesorder',soId,'custbody_pickup_date');
            nlapiLogExecution('DEBUG','pickup_date=', pickup_date);
            if (pickup_date) {
                    var vendorPeriod = getVendorPeriod(donorID, pickup_date);
              if(vendorPeriod.venPeriod) {
                    soTrans.setFieldValue('custbody_vendor_period', vendorPeriod.venPeriod);
              }
              if(vendorPeriod.venIntid) {
                    soTrans.setFieldValue('custbody_vendor_fiscal_record', vendorPeriod.venIntid);
              }
             }
         }
        	//Added by Elizabeth for ticket #5506
			var pickupdate = soTrans.getFieldValue('custbody_pickup_date');
              ...
              soTrans.setFieldValue('custbody_order_status', receipt_status);
				...
				soTrans.setFieldValue('custbody_receipt_received_date',new Date());
					...
					if ((orderType!='1') && (orderType!='2') && (orderType!='7')) {
						if (pickupdate == null) {
							soTrans.setFieldValue('custbody_pickup_date', soTrans.getFieldValue('custbodycustbody_requested_pickup_date'));
					}
				}
				if (stType =='edit')
				{
	        ...
	        arrParamSO['custscript_transtype']           = 'salesorder';
            arrParamSO['custscript_transintid']    = soId;
        	arrParamSO['custscript_transid'] = soTrans.getFieldValue('tranid');
	...
	// 
//Added by Thilaga for ticket #6540
  if(orderType!='6'){
        var status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger',arrParamSO);
         nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
         while(status!='QUEUED'){
               status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger_2',arrParamSO);
           nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
           if(status!='QUEUED'){
             status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger_3',arrParamSO);
           nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
           }
           if(status!='QUEUED'){
             status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger_4',arrParamSO);
           nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
           }
             if(status!='QUEUED'){
             status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger_5',arrParamSO);
           nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
               }
           if(status!='QUEUED'){
             status = nlapiScheduleScript('customscript_scheduled_order_email_trigg','customdeploy_scheduled_trigger',arrParamSO);
           nlapiLogExecution('EMERGENCY','Scheduled Script Called 2','Scheduled Script Queue Status= ' + status);
           }
         }
       }	
    ...
    return true;
        ...
        function getVendorPeriod(donorId, pickupDate) {
            ...
            var venInternalID = '';
          	...
          	nlapiLogExecution('DEBUG','pickDate=', pickDateStr);
          	//var formatPickDate = nsformat.format({ value: pickDate, type: nsformat.Type.DATE });
            //log.debug('formatPickDate=', formatPickDate);
   			var columns = new Array();
   			columns[0] = new nlobjSearchColumn('custrecord_fiscal_period');
   			columns[1] = new nlobjSearchColumn('custrecord_fiscal_year');
   			columns[2] = new nlobjSearchColumn('internalid');
            var filters = new Array();
          	filters[0] =new nlobjSearchFilter( 'custrecord_vendor', null, 'anyof', donorId );
          	filters[1] =new nlobjSearchFilter( 'custrecord_date_text', null, 'is', pickDateStr );
            var searchResults = nlapiSearchRecord('customrecord_ven_fiscal_dates', null, filters, columns);
            
          if(searchResults) {
            for (var i = 0; i < searchResults.length; i++){
                var fiscalPeriod = searchResults[i].getValue('custrecord_fiscal_period');
              	var fiscalYear = searchResults[i].getValue('custrecord_fiscal_year');
              	vendorPeriod = fiscalYear + '_pd_' + fiscalPeriod;
              	venInternalID = searchResults[i].getValue('internalid');
              	venFiscObj.venPeriod = vendorPeriod;
              	venFiscObj.venIntid = parseInt(venInternalID);
          	    nlapiLogExecution('DEBUG','vendorid=' + venInternalID);
          		nlapiLogExecution('DEBUG','vendorPeriod=' + vendorPeriod + venInternalID);
            	return venFiscObj;
            }
          }
            	return venFiscObj;
