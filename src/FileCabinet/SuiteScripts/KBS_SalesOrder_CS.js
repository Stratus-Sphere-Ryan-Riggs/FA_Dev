...
define(
    ...
    function (
        ...
        function reopenOrderClick() {
            var suiteUrl;

            var options = {
                title: 'Confirm Re-Opening',
                message: 'Are you sure you want to Re-Open this Sales Order and its lines? If yes, press OK. If no, press Cancel.'
            };

            function success(result) {
                if (result) {
                    suiteUrl = nsUrl.resolveScript({
                        scriptId: 'customscript_kbs_reopensalesorder_sl',
                        deploymentId: 'customdeploy_kbs_reopensalesorder_sl',
                    ...
                    if (pickuplocationText) {
                        if (povendor) {
                            ...
                            if (searchResults && searchResults.length > 0) {
                                ...
                                memberPrice = searchResults[0].getValue({
                                    name: 'custrecord_vwp_memberprice'
                                }); // Add an Inactive Check
                              ...
                              if (!memberPrice) {
                                ...
                                currentRecord.setCurrentSublistValue({
                                    sublistId: 'item',
                                    fieldId: 'custcol_kbs_memberdiscrate',
                                    value: memberPrice,
                                    ignoreFieldChange: true,
                                    fireSlavingSync: false
                                });
        ...
        function postSourcing(context) {
            ...
            if (sublistType === 'item') {
                ...
                if (sublistFieldId === 'item') {
                    bodyFieldVendor = currentRecord.getValue('custbody_opportunity_donor');
                  	lineFieldVendor = currentRecord.getCurrentSublistValue({
                            sublistId: 'item',
                            fieldId: 'povendor'
                        });
                    if (bodyFieldVendor && (bodyFieldVendor !== lineFieldVendor)) {
                        currentRecord.setCurrentSublistValue({
                            sublistId: 'item', fieldId: 'povendor', value: bodyFieldVendor, ignoreFieldChange: false
                        });
                    }
                    // If order type is Grocery (id = 1) then set the Vendor code to vendor on item record
                    orderType = currentRecord.getValue('custbody_order_type');
        ...
        */
        ...
        return {
            ...
            reopenOrderClick: reopenOrderClick,
