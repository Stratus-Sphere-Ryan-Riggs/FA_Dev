...
function schRecivePurchaseOrderFA() {
	try {
      ...
      if(poList != null){
        intNumToProcess = poList.length;
      	...
      	var intMaxNumToProcess = parseInt('50');
	...
	for (var i = 0; poList != null && i < intNumToProcess; i++) {
        try {
			...
			var soOrderType    = nlapiLookupField('salesorder', SOid, 'custbody_order_type');
			if(soOrderType === '2') {
				daysForAutoReceipting = CONTEXT.getSetting('SCRIPT', 'custscriptdaysautoreceipting');
			}
			else {
				daysForAutoReceipting = CONTEXT.getSetting('SCRIPT', 'custscriptdaysautoreceiptingfa_grocery');
			}
              ...
              //Edited for task 6325 by Thilaga
				...
				if (shippingMethodVA.indexOf(FArrangedTrans) != -1) {
					if(soOrderType === '2') {
						...
						daysForAutoReceipting = CONTEXT.getSetting('SCRIPT', 'custscript_daysautoreceipting_va_grocery');
					}
				...
				if (!isEmpty(deliveryDate)) {
					...
					for (var j = 1;
						...
						if (SORecord) {
							var customerId;
							var customerArray = ['custbody_drop_off_member_1', 'custbody_drop_off_member_2', 'custbody_drop_off_member_3', 'custbody_drop_off_member_4'];
							for (var xx = 0; xx < customerArray.length; xx += 1) {
								customerId = SORecord.getFieldValue(customerArray[xx]);
								if (customerId) {
									var customerAutoRecepting = nlapiLookupField('customer', customerId, 'custentity_kbs_disable_member_autoreceiv');
									if (customerAutoRecepting === 'T') {
										masterDisableAutoReceiting = true;
									}
								}
							}
...
//							var vendorID = SORecord.getLineItemValue('item', 'povendor', j);
        ...
        nlapiLogExecution('ERROR', error + POid);
      //add IF statement for this particular error only
      if (error.message == 'You must enter at least one line item for this transaction.') {
        var poHasIssues = nlapiSubmitField('purchaseorder', POid, 'custbody_order_has_issues', 'T');
        var poHasIssuesReason = nlapiSubmitField('purchaseorder', POid, 'custbody_order_issues', 'Requires Manual Receipting');
      }
    }
}
      ...
      for (var i = 0; donationList != null && i < intNumToProcess; i++) {
			...
			if (sosProcessed.indexOf(SOid) == -1) {
                ...
                var reqPickupDate = donationList[i].getValue('custbodycustbody_requested_pickup_date');
                var actualPickupDate = donationList[i].getValue('custbody_pickup_date');
				...
				if (!isEmpty(outByDate)) {
                  ...
                  var soDonor = donationList[i].getValue('custbody_opportunity_donor');
					...
					if (SOid) {
							...
							var customerArray = ['custbody_drop_off_member_1', 'custbody_drop_off_member_2', 'custbody_drop_off_member_3', 'custbody_drop_off_member_4'];
							for (var xx = 0; xx < customerArray.length; xx += 1) {
								customerId = donationList[i].getValue(customerArray[xx]);
								if (customerId) {
									var customerAutoRecepting = nlapiLookupField('customer', customerId, 'custentity_kbs_disable_member_autoreceiv');
									if (customerAutoRecepting === 'T') {
										masterDisableAutoReceiting = true;
  ...
  if (!masterDisableAutoReceiting) {
						...
						if (days >= daysForDonationAutoReceipting) {
                          ...
                          if (trecord.getLineItemCount('item') != 0) {
                            ...
                            //added by Elizabeth for DevOps 12185
                            if(actualPickupDate == null) {
                                nlapiSubmitField('salesorder', SOid, 'custbody_pickup_date', reqPickupDate);
                            }
                        }else{
                          bReschedule = false;
                          break;
