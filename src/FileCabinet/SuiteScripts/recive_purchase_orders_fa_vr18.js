...
function schRecivePurchaseOrderFA() {
	try {
		...
		var intNumToProcess = poList.length;
      	var intMaxNumToProcess = parseInt('250');
		...
		for (var i = 0; poList != null && i < intNumToProcess; i++) {
			...
			if (posProcessed.indexOf(POid) == -1) {
				...
				if (shippingMethodVA.indexOf(FArrangedTrans) != -1) {
					...
					deliveryDate = poList[i].getValue('custbody_release_date', 'createdFrom');
															...
															if (POquantity == 0) {
																...
																for (var itemLines = ifLines; itemLines > 0; itemLines--) {
                                                                  ...
                                                                  if (SORecord.getFieldValue('custbody_pickup_date') == null) {
                                                                  	...
                                                                  	var pickup = nlapiSubmitField('salesorder', SOid, 'custbody_pickup_date', SORecord.getFieldValue('custbodycustbody_requested_pickup_date'));
