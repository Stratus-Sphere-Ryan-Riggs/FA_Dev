...
var SS_SALES_ORDERS = 'customsearch_donation_orders_to_receip_2';

function schReceiveDonationSO() {
	try {
		var posProcessed = [];
      	var sosProcessed = [];
      ...
      for (var i = 0; donationList != null && i < intNumToProcess; i++) {
			...
			var daysForDonationAutoReceipting = CONTEXT.getSetting('SCRIPT', 'custscript_days_fa_arr_donation_receipt');
			if (sosProcessed.indexOf(SOid) == -1) {

				var actualDeliveryDate = donationList[i].getValue('custbody_actual_carrier_delivery_date');

				
                nlapiLogExecution('DEBUG', 'actualDeliveryDate', actualDeliveryDate);
				if (!isEmpty(actualDeliveryDate)) {
  ...
  if (!masterDisableAutoReceiting) {
						...
						var date = new Date();
						var days = getBusinessDatesCount(actualDeliveryDate, date);
						nlapiLogExecution('DEBUG', 'Step 4');
						nlapiLogExecution('DEBUG', 'date', date);
						nlapiLogExecution('DEBUG', 'outByDate', actualDeliveryDate);
