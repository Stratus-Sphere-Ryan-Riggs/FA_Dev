...
function updateRelatedRecords() {
 //   try {
        ...
        var isReleaseDateChanged = context.getSetting('SCRIPT', 'custscript_isreleasedatechanged');
  		...
  		var isProjectCodeChanged = context.getSetting('SCRIPT', 'custscript_isprojectcodechanged');
      	var isFundChanged = context.getSetting('SCRIPT', 'custscript_isfundchanged');
      ...
      var isPickUpSequenceChanged = context.getSetting('SCRIPT', 'custscript_is_line_pickup_num_changed');
       var isDropOffSequenceChanged = context.getSetting('SCRIPT', 'custscript_is_line_dropoff_num_changed');
  		var isLineRateChanged = context.getSetting('SCRIPT', 'custscript_is_line_rate_changed');
  		var isPoAdmFeeRateChanged = context.getSetting('SCRIPT', 'custscript_is_line_poadminfee_changed');
  ...
  nlapiLogExecution('DEBUG', 'idSalesOrder'+'isDropOffLocChanged', JSON.stringify(isDropOffLocChanged));
        var icFilters = new Array();
        var arrRelatedRecords = new Array();
      if(isReleaseDateChanged||isReqPickUpDateChanged||isPoVenCommentsChanged||isShippingMethodChanged||isMemberPONumChanged || isProjectCodeChanged || isFundChanged || isLineQtyChanged ||isPickUpLocChanged|| isDropOffLocChanged|| isPickUpSequenceChanged|| isDropOffSequenceChanged || isLineRateChanged||isPoAdmFeeRateChanged){
        ...
        if (icSearchResults != null && icSearchResults.length != 0) {
            for (var i = 0; i < icSearchResults.length; i++) {
                ...
                if ((tranType == 'PurchOrd') || (tranType == 'CustInvc') || (tranType == 'ItemShip')) {
                    var relatedRecord = {
                        tranid: tranId,
                        type: tranType,
                      	vendor:vendor
      ...
      nlapiLogExecution('DEBUG', 'arrRelatedRecords=' + arrRelatedRecords );
        ...
        var fields = ['custbody_order_type','custbody_release_date','custbodycustbody_requested_pickup_date','custbodycustbody_po_vendor_comments','custbody_shipping_method_code','otherrefnum','custbody_cseg_projects_cseg','location'];
        ...
        var projectCode = columns.custbody_cseg_projects_cseg;
      	var fund = columns.location;
      	var sendPOModEmail = false;
        var tranChanges = '';
        var transLineChange = '';
        nlapiLogExecution('DEBUG', 'Release Date', releaseDate + ' isReleaseDateChanged ' + isReleaseDateChanged+ 'Req pick up date' + reqPickupDate+ 'isReqPickUpDateChanged'+isReqPickUpDateChanged+' poComments '+poComments+ 'isMemberPONumChanged '+isMemberPONumChanged+ 'isProjectCodeChanged '+isProjectCodeChanged+ 'isFundChanged '+isFundChanged);
        ...
        if(isProjectCodeChanged){
          for (var i = 0; i < arrRelatedRecords.length; i++) {
                if (arrRelatedRecords[i].type == 'CustInvc') {
                    nlapiSubmitField('invoice', arrRelatedRecords[i].tranid, 'custbody_cseg_projects_cseg', projectCode, false);
                }
                if (arrRelatedRecords[i].type == 'PurchOrd') {
                    nlapiSubmitField('purchaseorder', arrRelatedRecords[i].tranid, 'custbody_cseg_projects_cseg', projectCode, false);
                }
            }
        }
        if(isFundChanged){
          for (var i = 0; i < arrRelatedRecords.length; i++) {
          			nlapiLogExecution('DEBUG','Type',arrRelatedRecords[i].type);
                if (arrRelatedRecords[i].type == 'CustInvc') {
                    nlapiSubmitField('invoice', arrRelatedRecords[i].tranid, 'location', fund, false);
                }
                if (arrRelatedRecords[i].type == 'PurchOrd') {
                    nlapiSubmitField('purchaseorder', arrRelatedRecords[i].tranid, 'location', fund, false);
                }
            }
        }
        ...
        if(isPickUpSequenceChanged){
          var splitString = isPickUpSequenceChanged.split(',');
          var changeList = [];
          nlapiLogExecution('DEBUG','isPickUpSequenceChanged','splitString= ' + splitString+" length="+splitString.length);
          for(var i=0;i<splitString.length-1;i++){
            var splitDetails = splitString[i].split(':');
            nlapiLogExecution('DEBUG','isPickUpSequenceChanged details','splitDetails= ' + splitDetails+" length="+splitDetails.length);
            var changeLineDetails={
              magRef:splitDetails[0],
              oldPickUpSequence:splitDetails[1],
              newPickUpSequence:splitDetails[2],
              item:splitDetails[3]
            };
            changeList[i]=changeLineDetails;
          }
            for (var i = 0; i < arrRelatedRecords.length; i++) {
                if (arrRelatedRecords[i].type == 'PurchOrd') {
                  ...
                  for(var j=0;j<changeList.length;j++){
                   ...
                   nlapiLogExecution('DEBUG','lineNum=',lineNum);
                  if(lineNum!=-1){
                    change = true; poRec.setLineItemValue('item','custcol_pickup_sequence',lineNum,changeList[j].newPickUpSequence);

                  }
                  }
                   // nlapiSubmitField('purchaseorder', arrRelatedRecords[i].tranid, 'custbody_trans_line_changes', transLineChange, false);
               if(change){
                 transLineChange = transLineChange + "<br>" + 'Pick up sequence is changed. Please check PO Pdf for details.'+"</br>";
                  poRec.setFieldValue('custbody_trans_line_changes',transLineChange);
               }
                   nlapiSubmitRecord(poRec);
               }
        ...
        if(isDropOffSequenceChanged){
          var splitString = isDropOffSequenceChanged.split(',');
          var changeList = [];
          nlapiLogExecution('DEBUG','isDropOffSequenceChanged','splitString= ' + splitString+" length="+splitString.length);
          for(var i=0;i<splitString.length-1;i++){
            var splitDetails = splitString[i].split(':');
            nlapiLogExecution('DEBUG','isDropOffSequenceChanged details','splitDetails= ' + splitDetails+" length="+splitDetails.length);
            var changeLineDetails={
              magRef:splitDetails[0],
              oldDropOffSequence:splitDetails[1],
              newDropOffSequence:splitDetails[2],
              item:splitDetails[3],
              newDropOffMember:splitDetails[4]
            };
            changeList[i]=changeLineDetails;
                ...
                if (arrRelatedRecords[i].type == 'PurchOrd') {
                 // transLineChange = '';
                  ...
                  for(var j=0;j<changeList.length;j++){
                   ...
                   nlapiLogExecution('DEBUG','lineNum=',lineNum);
                  if(lineNum!=-1){
                 	poRec.setLineItemValue('item','custcol_drop_off_sequence',lineNum,changeList[j].newDropOffSequence);
                    change = true;
                  }
                   ...
                   // nlapiSubmitField('purchaseorder', arrRelatedRecords[i].tranid, 'custbody_trans_line_changes', transLineChange, false);
                   if(change){
                    transLineChange = transLineChange + "<br>" + 'Drop off sequence is changed. Please check PO Pdf for details'+"</br>";
                    poRec.setFieldValue('custbody_trans_line_changes',transLineChange);
                    }
                  nlapiSubmitRecord(poRec);
               }
        ...
        if(isLineRateChanged){
          ...
          for(var i=0;i<splitString.length-1;i++){
            ...
            for (var i = 0; i < arrRelatedRecords.length; i++) {
                ...
                if (arrRelatedRecords[i].type == 'PurchOrd') {
                  var poRec = nlapiLoadRecord('purchaseorder',arrRelatedRecords[i].tranid);
                  if(poRec.getFieldValue('status')!='Fully Billed'){
                    for(var j=0;j<changeList.length;j++){
                      var lineNum= poRec.findLineItemValue('item','custcol_mag_ref_no',changeList[j].magRef);
                    if(lineNum!=-1){
                          poRec.setLineItemValue('item','rate',lineNum,changeList[j].newRate);
                    }
                  }
                    nlapiSubmitRecord(poRec);
                  }
        ...
        if(isPoAdmFeeRateChanged){
          nlapiLogExecution('DEBUG','isPoAdmFeeRateChanged','= ' + isPoAdmFeeRateChanged);
          var splitString = isPoAdmFeeRateChanged.split(',');
          var changeList = [];
          nlapiLogExecution('DEBUG','isPoAdmFeeRateChanged','splitString= ' + splitString+" length="+splitString.length);
          for(var i=0;i<splitString.length-1;i++){
            var splitDetails = splitString[i].split(':');
            nlapiLogExecution('DEBUG','isPoAdmFeeRateChanged details','splitDetails= ' + splitDetails+" length="+splitDetails.length);
            var changeLineDetails={
              magRef:splitDetails[0],
              poAdmFeeNewRate:splitDetails[1]
            };
            changeList[i]=changeLineDetails;
          }
          for (var i = 0; i < arrRelatedRecords.length; i++) {
                if (arrRelatedRecords[i].type == 'ItemShip') {
                  nlapiLogExecution('DEBUG','before ItemShip load','before load');
                  var itemFulfillRec = nlapiLoadRecord('itemfulfillment',arrRelatedRecords[i].tranid);

                  	for(var j=0;j<changeList.length;j++){
                    	var lineNum= itemFulfillRec.findLineItemValue('item','custcol_mag_ref_no',changeList[j].magRef);

                   	nlapiLogExecution('DEBUG','lineNum=',lineNum);
                  	if(lineNum!=-1){
                 				itemFulfillRec.setLineItemValue('item','custcol_produce_adm_fee_qty',lineNum,changeList[j].poAdmFeeNewRate);
                   }

                  }
                   nlapiSubmitRecord(itemFulfillRec);
                }
          }
 ...
 poRec.setCurrentLineItemValue('item','custcol_cases_per_pallet',newLineItemsList[i].qtyPerPallet);         poRec.setCurrentLineItemValue('item','custcol_admin_fee_vendor',newLineItemsList[i].adminFeeVendor);      poRec.setCurrentLineItemValue('item','custcol_item_gross_weight',newLineItemsList[i].itemGrossWt);     poRec.setCurrentLineItemValue('item','custcol_total_pounds',newLineItemsList[i].totalPounds);       poRec.setCurrentLineItemValue('item','custcol_total_weight',newLineItemsList[i].totalGrossWt);       poRec.setCurrentLineItemValue('item','custcolfa_vendor_item_name',newLineItemsList[i].vendorItemName);   poRec.setCurrentLineItemValue('item','custcol_vendor_item_no',newLineItemsList[i].vendorItemNumber);       poRec.setCurrentLineItemValue('item','custcol_pickup_sequence',newLineItemsList[i].pickupSequence);       poRec.setCurrentLineItemValue('item','custcol_drop_off_sequence',newLineItemsList[i].dropoffSequence);       poRec.setCurrentLineItemValue('item','custcol_mag_ref_no',(newLineItemsList[i].magRefNum).toFixed(0));
