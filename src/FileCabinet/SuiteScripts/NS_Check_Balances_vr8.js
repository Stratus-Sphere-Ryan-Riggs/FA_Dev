...
function getCustomerBalances(recIds, itemId, poID) {
  ...
  var isFinanceApproved = nlapiGetFieldValue('custbody_finance_approved');
	nlapiLogExecution('Debug', 'Old Record  ' + oldSalesOrder.getFieldValue('tranid'));
		...
		if ((customerId                  == oldSalesOrder.getFieldValue('entity')) &&
			...
			{	
	         nlapiLogExecution('Debug', 'IN retuern no issue  ');
              //commenting as a workaround to run this script on edit as well
			// return creditHol;
		    }	 
    ...
    if (daysRelease < daysVar && (isFinanceApproved=='' || isFinanceApproved=='F'))
	{	
	  if ((parseFloat(headerAmt) + parseFloat(custOutStanding.balance) + parseFloat(soOutStanding) > parseFloat(custOutStanding.creditlimit)) || (custOutStanding.custentity_fa_credit_status == CREDIT_HOLD) ) { 
         ...
         //rf change 9-12-2016
        //added Thilaga
        nlapiLogExecution('DEBUG','Setting Thilaga ','1');
        nlapiLogExecution('DEBUG','Setting Thilaga Value',(parseFloat(headerAmt) + parseFloat(custOutStanding.balance) + parseFloat(soOutStanding)));
                nlapiSetFieldValue('custbody_header_member_bal',(parseFloat(headerAmt) + parseFloat(custOutStanding.balance) + parseFloat(soOutStanding)));
         nlapiSetFieldValue('custbody_mem_credit_limit',customerId);
        if(parseFloat(custOutStanding.balance) + parseFloat(soOutStanding) > parseFloat(custOutStanding.creditlimit)){
    ...
    for (var i = 1; i <= lineCt; i++) {
        ...
        if (usedArray.indexOf(nlapiGetLineItemValue('item', 'custcol_member_bank', i)) < 0) {
          ...
          //Added for ticket #4996 by Thilaga
          var billToMember = nlapiGetLineItemValue('item', 'custcol_bill_to_member', i);
            if (!custLineId) {
                continue;
            }
           //Added for ticket #4996 by Thilaga
          if(billToMember=='F'){
            nlapiLogExecution('Debug', 'Bill To Member  ' + billToMember);
            continue;
          }
            ...
            if (daysRelease < daysVar && (isFinanceApproved=='' || isFinanceApproved=='F'))
           	{	
			  if ((parseFloat(compareTotal) + parseFloat(lineCustOutstanding.balance)+parseFloat(soLineOutstanding) > lineCustOutstanding.creditlimit) || (lineCustOutstanding.custentity_fa_credit_status == CREDIT_HOLD)) {
                //nlapiSubmitField('customer', customerId, ['custentity_fa_credit_status', 'creditholdoverride', 'isinactive'], [CREDIT_HOLD, "ON", "T"]);
                //rf change 9-12-2016
                //added Thilaga
                nlapiLogExecution('DEBUG','Setting Thilaga ','2');
                nlapiLogExecution('DEBUG','Setting Thilaga Value',(parseFloat(compareTotal) + parseFloat(lineCustOutstanding.balance)));
                nlapiSetFieldValue('custbody_header_member_bal',(parseFloat(compareTotal) + parseFloat(lineCustOutstanding.balance)));
                nlapiSetFieldValue('custbody_mem_credit_limit',custLineId);
                if(parseFloat(lineCustOutstanding.balance)+parseFloat(soLineOutstanding) > lineCustOutstanding.creditlimit){
              ...
              //  updateRelatedDropShipPOs();
                                                           nlapiLogExecution('DEBUG','Setting Thilaga ','End');
              }
			}			 
            usedArray.push(nlapiGetLineItemValue('item', 'custcol_member_bank', i))
        }
    }
                                                          nlapiLogExecution('DEBUG','Setting Thilaga ','before return'+creditHol);
    return creditHol;
 }catch(e)
    {
