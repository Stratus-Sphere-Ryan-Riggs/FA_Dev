  ...
  if(orderType == produce){
    nlapiLogExecution('DEBUG','Status>> fields ','Status= ' + recTrans.getFieldValue('status') + ' trandate=' + recTrans.getFieldValue('trandate') + ' effective_date=' + effective_date+' end_date='+end_date);
         ...
         //   nlapiLogExecution('DEBUG','Befoore setting freight subsidy item ','incl_freight_subsidy= ' + incl_freight_subsidy + ' ' +'shippingMethod='+ shippingMethod+' tranDate='+recTrans.getFieldValue('trandate')+' status='+recTrans.getFieldValue('status')+' effective_date='+effective_date+' end_date='+end_date+' date compare='+(nlapiStringToDate(recTrans.getFieldValue('trandate'))>nlapiStringToDate(effective_date)));			             
             if(incl_freight_subsidy == 'T' && freight_sub_amount!=0 && Eval.inArray(shippingMethod, ARR_SHIP_TYPES) && recTrans.getFieldValue('status')!='Billed' && recTrans.getFieldValue('status')!='Closed' && (nlapiStringToDate(salesOrdDate)>=effective_date && nlapiStringToDate(salesOrdDate)<=end_date)){
