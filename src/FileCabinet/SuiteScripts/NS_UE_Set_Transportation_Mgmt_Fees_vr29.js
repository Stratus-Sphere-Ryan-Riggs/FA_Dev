     ...
     // nlapiLogExecution('DEBUG','Script Exit: Invalid Operation Type','stType= ' + defaultLocation + ' ' + paramcarriertype);
	  ...
	  for (var i = 1; i <= recTrans.getLineItemCount('item'); i++ ) 
      {
		...
		if ((tempA == 1 || tempB == 1) && tempC == 1)
		{
         ...
         nlapiLogExecution('DEBUG','Prorate Transport ','stCost= ' + totalcost + ' TtlQuantity ' + ttlQuantity);
		 if (ttlQuantity > 0 && totalcost > 0)
		 {
			 var tsCostPerItem = parseFloat(totalcost/ttlQuantity);
	 	     ...
	 	     for (var i = 1; i <= recTrans.getLineItemCount('item'); i++ ) 
             {
		           ...
		           if (itemChannel != adminChannel)
		           {
		             ...
		             recTrans.setLineItemValue('item', 'custcol_adj_invoice_rate', i,nlapiFormatCurrency(tsCostPerItem));
		           }
			 }		   
