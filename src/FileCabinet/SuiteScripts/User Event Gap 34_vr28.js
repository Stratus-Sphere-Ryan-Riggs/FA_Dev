...
//Added for ticket #6286 by Thilaga
		...
		var seafoodProgram = nlapiGetContext().getSetting('SCRIPT', 'custscript_seafood_program2');
      	var donationProgram = nlapiGetContext().getSetting('SCRIPT', 'custscript_donation_program');
...
//				nlapiLogExecution('DEBUG', 'body : ' + body);
      ...
      var project = nlapiLookupField('salesorder', thisSalesOrderID, 'custbody_cseg_projects_cseg');
        ...
        //updated below by Elizabeth for DevOps Tasks 10302 and 11271
		var salesOrderOfferNumber = thisSalesOrder.getFieldValue('custbody_product_offer_no');
		var salesOrderReceiptDueDate = thisSalesOrder.getFieldValue('custbody_receipt_due_date');
        ...
        //updated below by Elizabeth for DevOps Tasks 10302 and 11271
        if(salesOrderOfferNumber) {
		purchaseOrder.setFieldValue('custbody_product_offer_no', salesOrderOfferNumber);
        }
        if(salesOrderReceiptDueDate) {
		purchaseOrder.setFieldValue('custbody_receipt_due_date', salesOrderReceiptDueDate);
        }
        ...
        if (thisOrderType != disaster) {
        	if(thisOrderType == grocery || thisOrderType == produce){
		...
		if(!isEmpty(actualCost) && (!isEmpty(actualCost_sur) || !isEmpty(actualCost_linehaul))){
           ...
           var toleranceLow = nlapiGetContext().getSetting('SCRIPT',
			...
			if(change != 0  && thisOrderType ==produce){
