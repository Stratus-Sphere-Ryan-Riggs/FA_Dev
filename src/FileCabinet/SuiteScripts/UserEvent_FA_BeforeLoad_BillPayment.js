...
* 
* @version 1.0
* 1) On Create of a new bill payment - find the apply entries and set the dept, class and Loocations
*/
function beforeLoad_BillPayment(stType)
{
   try
   {
       nlapiLogExecution('DEBUG','beforeLoad_BillPayment','|------------SCRIPT STARTED------------||' + stType);
        ...
        for (var i = 1; i <= nlapiGetLineItemCount('apply'); i++ ) 
        {
			...
			if (applyLine != '' && applyLine != null)
			{
				...
				if (type_v == 'VendBill')
				{
					var recTrans = nlapiLoadRecord('vendorbill', applyLine);
					var dept_v  = recTrans.getLineItemValue('item','department',i);
					var class_v = recTrans.getLineItemValue('item','class',i);
					var loc_v   = recTrans.getLineItemValue('item','location',i);
