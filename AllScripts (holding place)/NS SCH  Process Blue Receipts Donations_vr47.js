...
function createItemRecords(itemName, arrayItems, itemInSearch, suffixForField, donationID, itemIndex, ARR_STORAGE_TYPES, itemDate1)
{
	...
	if(objItem.item && objItem.item != '')
	{
			...
			//Edited to ticket #5435
	 		var itemHandling = 	nlapiLookupField('noninventoryitem',objItem.itemID,'custitem_storage_requirements',true);
            nlapiLogExecution('debug', 'itemHandling' + objItem.itemID+' '+itemHandling);
		   ...
		   nlapiLogExecution('debug', 'Item Date ' + itemIndex + ' date ' +  itemDate1);
			...
			{	
			 ...
			 if(objItem.handCode == '' || objItem.handCode == null)
			{	
              objItem.handCode = itemHandling;
              nlapiSubmitField('customrecord_donation_import', donationID, 'custrecord_item_handling'+suffixForField, itemHandling);
			//  exception('Missing Item Handling Code for #' + itemIndex + ' Exception', donationID, true);
			 // itemsException = true;
               nlapiLogExecution('debug', 'objItem.handCode',' '+objItem.handCode);
			}
         ...
         nlapiLogExecution('EMERGENCY', 'After Opp create Date/Time', nlapiDateToString(currentDate,'datetimetz')); 
			if(oppID)
			{
				...
				try
				{
                  ...
                  filterWH[2]= new nlobjSearchFilter('isinactive', null, 'is', 'F');
...
function lookWarehouse(donationID, memberDonor) 
{
  ...
  //Added by Thilaga for ticket #6363
  if(warehouseName){
    filters.push(new nlobjSearchFilter('custrecord_entity_warehouse_name', null, 'is', warehouseName));	
  }
	if(warehouseCode)
	{
