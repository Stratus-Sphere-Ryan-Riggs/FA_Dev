...
function beforesubmit_ordervalues(stType)
{
  ...
  if(stType=='edit')
	...
	if(orderType==2 && shippingMethod==2){
		...
		nlapiSetCurrentLineItemValue('item', 'quanity', '1');
	...
	if(stType=='edit'){
      ...
      if(linenumber!=-1 && (recOldOrder.getFieldValue('custbody_actual_freight_cost')!=rate)){
       ...
       if ( nlapiGetFieldValue('custbody_total_pounds') != oldRecord.getFieldValue('custbody_total_pounds')) 
	   {
		   ...
		   {	   
			 ...
			 if (poundChange >= .10 && (Eval.inArray(orderType, ARR_DONATION_TYPES)))
		     {
			  ...
			  changeText = changeText + "<br>" + 'The total pounds has changed from: ' + oldPoundsT + ' to: ' + newPounds + "</br>";
		      //nlapiSetFieldValue('custbody_order_status',stOrderModified)
 		      recModified = true;
			 } 
