...
function aftersubmit_addfees(stType)
{
  ...
  //Added for ticket #7140
	...
	//Added for ticket #5096 by Thilaga
		if(orderType!=grocery && orderType!=produce){
          var producecost = parseFloat((actFreight + actFreight_sur));
			totalTransportCost = producecost; 
   		    totalcost   = parseFloat(producecost + accessorial_cost);
		    recTrans.setFieldValue('custbody_total_freight_cost',nlapiFormatCurrency(totalcost));
  ...
  if(orderType == produce){
          if(foundSubsidy=='-1'){
         //   nlapiLogExecution('DEBUG','Befoore setting freight subsidy item ','incl_freight_subsidy= ' + incl_freight_subsidy + ' ' +'shippingMethod='+ shippingMethod+' tranDate='+recTrans.getFieldValue('trandate')+' status='+recTrans.getFieldValue('status')+' effective_date='+effective_date+' end_date='+end_date+' date compare='+(nlapiStringToDate(recTrans.getFieldValue('trandate'))>nlapiStringToDate(effective_date)));			
             var salesOrdDate = recTrans.getFieldValue('trandate');
             if(incl_freight_subsidy == 'T' && freight_sub_amount!=parseFloat('0') && Eval.inArray(shippingMethod, ARR_SHIP_TYPES) && recTrans.getFieldValue('status')!='Billed' && (nlapiStringToDate(salesOrdDate)>=nlapiStringToDate(effective_date)) && (nlapiStringToDate(salesOrdDate)<=nlapiStringToDate(end_date))){
              // nlapiLogExecution('DEBUG','inside create subsidy item','freight_sub_item='+freight_sub_item);
             	recTrans.selectNewLineItem('item');
           		recTrans.setCurrentLineItemValue('item','item',freight_sub_item); 
           		recTrans.setCurrentLineItemValue('item','quantity','1'); 
           		recTrans.setCurrentLineItemValue('item','location',  defaultLocation); 
           		recTrans.setCurrentLineItemValue('item','class',     defaultClass);
               	recTrans.setCurrentLineItemValue('item','custcol_cseg_projects_cseg',     '5065');
