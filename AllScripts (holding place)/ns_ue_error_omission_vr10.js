...
var stTransportHighVariance = context.getSetting('SCRIPT', 'custscript_has_issue_variance_threshold');
//Added by Thilaga for ticket #6580
var stProduceVariance = context.getSetting('SCRIPT', 'custscript_produce_variance_threshold');
var stGroceryVariance = context.getSetting('SCRIPT', 'custscript_grocery_variance_threshold');
var stAdminFeeVariance = context.getSetting('SCRIPT', 'custscript_adminfee_variance_threshold');
var stTransportVariance = context.getSetting('SCRIPT', 'custscript_transport_variance_threshold');
...
function afterSubmit_error_ommission(type) {
    ...
    try {
        ...
        */
		...
		objData.issueVariance = parseFloat('0');
      //Added by Thilaga for ticket #6580
        objData.groceryVariance = stGroceryVariance;
       objData.produceVariance = stProduceVariance;
       objData.transportVariance = stTransportVariance;
       objData.adminFeeVariance = stAdminFeeVariance;
		objData.transHighVariance = stTransportHighVariance;
        ...
        var poId = myBill.getLineItemValue('purchaseorders', 'id', 1);
        //if no po Exit
        //if (!poId) {
       //     return false;
       // }
		...
		//Edited for ticket #5343
        logExecution('DEBUG', 'stLoggerTitle', 'Pre po 1'+poId);
      var pOeOValue= 0.00;
        if (poId) {
          ...
          var poRec = nlapiLoadRecord('purchaseorder',poId);
            var salesOrdType = '';
          if(poRec.getFieldValue('custbody_associated_salesorder')!=null && poRec.getFieldValue('custbody_associated_salesorder')!=''){
          		salesOrdType = nlapiLookupField('salesorder',poRec.getFieldValue('custbody_associated_salesorder'),'custbody_order_type');
          }
            objData.orderType = orderType;
			objData.vendorBillType = nlapiGetFieldValue('custbody_vendor_bill_type');
          logExecution('DEBUG', 'stLoggerTitle', 'poRec and eo item' +poRec+' '+ poRec.findLineItemValue('item','item',stTransChainVarianceItem));
          if(salesOrdType=='1' &&  poRec.findLineItemValue('item','item',stTransChainVarianceItem)!='-1'){
            
           pOeOValue = poRec.getLineItemValue('item','amount',poRec.findLineItemValue('item','item',stTransChainVarianceItem));
            logExecution('DEBUG', 'stLoggerTitle', 'In Grocery and Transport E&o present' + poRec.findLineItemValue('item','item',stTransChainVarianceItem)+' '+pOeOValue);
            objData.eoValue= pOeOValue;
          } 
...
function addSummaryLine(proBill, objData, lineItem, amtDiff, Rcode, masterSO) {
    ...
    //logExecution('DEBUG', 'stLoggerTitle', 'IN Add Summary' + amtDiff + ' ' objData.eoValue+' '+ objData.tranVarAmt + ' '+ objData.supplyVarAmt );
  ...
  //Added by Thilaga
  var highVariance = parseFloat('0');
  if(parseInt(objData.orderType)==1){
    objData.issueVariance = parseFloat(0)-parseFloat(stGroceryVariance);
    highVariance = stGroceryVariance;
  }
  if(parseInt(objData.orderType)==2){
    objData.issueVariance = parseFloat(0)-parseFloat(stProduceVariance);
    highVariance = stProduceVariance;
  }
  if(parseInt(objData.orderType)==7){
    objData.issueVariance = parseFloat(0)-parseFloat(stTransportVariance);
    highVariance = stTransportHighVariance;
  }
  if(parseInt(objData.vendorBillType) == 1){
    objData.issueVariance = parseFloat(0)-parseFloat(stAdminFeeVariance);
    highVariance = stAdminFeeVariance;
  }
  nlapiLogExecution('DEBUG', "amtDiff>>highVariance>>objData.issueVariance", amtDiff+" >> "+highVariance+' >> '+objData.issueVariance);
    if (amtDiff > highVariance || amtDiff < objData.issueVariance) {
    //    proBill.setFieldValue('custbody_order_has_issues', 'T');
		//Added by Thilaga (commented Has issues for ticket #5114)
       nlapiLogExecution('DEBUG', "Approval status", proBill.getFieldValue('approvalstatus'));
      proBill.setFieldValue('approvalstatus',1);
      
      if(objData.eoValue!=null && objData.eoValue!='0.00'){
        var diff = parseFloat(amtDiff)-parseFloat(objData.eoValue);
        nlapiLogExecution('DEBUG', "diff>>highVariance>>objData.issueVariance", diff+" >> "+highVariance+' >> '+objData.issueVariance);
        if(diff <= highVariance && diff >= objData.issueVariance)		  {
          proBill.setFieldValue('approvalstatus',2);
        }
      }
...
function removeSummaryLine(proBill, objData) {
  ...
  //Added for fix 6535
  //if(objData.vendorBillType!='' && objData.vendorBillType!=null){
    proBill.setFieldValue('approvalstatus',2);
	//}
    nlapiSubmitRecord(proBill);
}
