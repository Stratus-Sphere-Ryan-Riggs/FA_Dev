...
function beforesubmit_ordervalues(stType)
{
    ...
    if (stType != 'edit' && stType != 'create' && stType != 'approve') {
        //do nothing
...
if(nlapiGetLineItemText('item', 'custcol_product_channel', i)!="Admin Fees") {     
    ...
    if (orderType == 1) {
        if (isEmpty(nlapiGetFieldValue('custbody_release_date')) || nlapiGetFieldValue('custbody_release_date')=='') {
            if (maxLeadDays > 0) {
                var today = new Date();
              ...
              var today = new Date();
              nlapiSetFieldValue('custbody_release_date', today);
            }
        }
    }
  ...
  var oldOutDate = nlapiStringToDate(recOldOrder ? recOldOrder.getFieldValue('custbody_outbydate') : null);
  var oldReleaseDate = nlapiStringToDate(recOldOrder ? recOldOrder.getFieldValue('custbody_release_date') : null);
  var newReleaseDate = nlapiStringToDate(nlapiGetFieldValue('custbody_release_date'));
  var oldActualDeliveryDate = nlapiStringToDate(recOldOrder ? recOldOrder.getFieldValue('custbody_actual_carrier_delivery_date') : null);
  var newActualDeliveryDate = nlapiStringToDate(nlapiGetFieldValue('custbody_actual_carrier_delivery_date'));
  nlapiLogExecution('DEBUG','for bug 5362 + oldOutDate', oldOutDate+" : newOutDate "+newOutDate);
  nlapiLogExecution('DEBUG','for bug 5362 + oldReleaseDate', oldReleaseDate+" : newReleaseDate "+newReleaseDate);
  if(Eval.inArray(orderType, ARR_OUTBYORDER_TYPES) && (oldReleaseDate!=newReleaseDate) && stType=='edit'){
    nlapiSetFieldValue('custbody_outbydate','');
	nlapiSetFieldValue('custbodycustbody_requested_pickup_date','');
  }
  var newOutDate = nlapiStringToDate(nlapiGetFieldValue('custbody_outbydate'));
  if(Eval.inArray(orderType, ARR_OUTBYORDER_TYPES) && (oldOutDate!=newOutDate) && stType=='edit'){
    nlapiSetFieldValue('custbody_receipt_due_date','');
  }
   
  if (Eval.inArray(orderType, ARR_RECEIPTBYORDER_TYPES)) {
		if((oldReleaseDate!=newReleaseDate) && (stType=='edit')){
               nlapiSetFieldValue('custbodycustbody_requested_pickup_date', '');
               }
        if (Eval.inArray(shipmentMethod, ARR_FASHIPR_TYPES)) {
            if ((oldActualDeliveryDate != newActualDeliveryDate) && (stType=='edit')) {
                nlapiSetFieldValue('custbody_receipt_due_date', '');
            }
        }
    }
	...
	if (Eval.inArray(orderType, ARR_OUTBYORDER_TYPES) && (isEmpty(nlapiGetFieldValue('custbody_outbydate')) || isEmpty(nlapiGetFieldValue('custbody_receipt_due_date')) || isEmpty(nlapiGetFieldValue('custbodycustbody_requested_pickup_date')))) {
      ...
      //Edited for ticket #7200 by Thilaga
            if ((nlapiStringToDate(nlapiGetFieldValue('custbody_release_date').valueOf())) < (nlapiStringToDate(nlapiGetFieldValue('trandate').valueOf()))) {
                today = nlapiGetFieldValue('trandate');
                nlapiSetFieldValue('custbody_release_date', today);
                newReleaseDate = nlapiGetFieldValue('custbody_release_date');
            } else {
...
if (isEmpty(nlapiGetFieldValue('custbody_outbydate'))) {
        ...
        if (isEmpty(nlapiGetFieldValue('custbody_receipt_due_date'))) {
           today = new Date(nlapiGetFieldValue('custbody_outbydate'));
            today = nlapiDateToString(today);
        ...
        if (Eval.inArray(orderType, ARR_RECEIPTBYORDER_TYPES)) {
            ...
            if (Eval.inArray(shipmentMethod, ARR_FASHIPR_TYPES)) {
                ...
                var oldReqPickupDate = recOldOrder ? recOldOrder.getFieldValue('custbodycustbody_requested_pickup_date') : null;
                var newReqPickupDate = nlapiGetFieldValue('custbodycustbody_requested_pickup_date');
                ...
                if (isEmpty(nlapiGetFieldValue('custbody_receipt_due_date'))) {
                    ...
                    today = nlapiGetFieldValue('custbodycustbody_requested_pickup_date');
                    // var newdate    = nlapiDateToString(nlapiAddDays(nlapiStringToDate(today), leadDays));
                    var newdateC = nlapiStringToDate(today);
                    var dayOfWeek = newdateC.getDay();
                    var validDate = false;
  ...
  // Check if releaseDate, out by date, release number or the line item pickup location or status changes - set to modified
	if(stType=='edit'){
       ...
       nlapiSetFieldValue('custbody_carrier_msg','');
	   ...
	   if ( (nlapiGetFieldValue('custbodycustbody_requested_deliv_date') !=  oldRecord.getFieldValue('custbodycustbody_requested_deliv_date')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbodycustbody_requested_deliv_date')) || !isEmpty(oldRecord.getFieldValue('custbodycustbody_requested_deliv_date')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbodycustbody_requested_deliv_date');
		   if  (isEmpty(nlapiGetFieldValue('custbodycustbody_requested_deliv_date')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Requested Delivery Date has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbodycustbody_requested_deliv_date') + "</br>";
           recCarrierMod = true;
       }
      
	   if ( (nlapiGetFieldValue('custbodycustbody_requested_pickup_date') !=  oldRecord.getFieldValue('custbodycustbody_requested_pickup_date')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbodycustbody_requested_pickup_date')) || !isEmpty(oldRecord.getFieldValue('custbodycustbody_requested_pickup_date')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbodycustbody_requested_pickup_date');
		   if  (isEmpty(nlapiGetFieldValue('custbodycustbody_requested_pickup_date')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Requested Pickup Date has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbodycustbody_requested_pickup_date') + "</br>";
           recCarrierMod = true;
       }
      
	  /* if ( (nlapiGetFieldValue('custbodycustbody_sched_delivery_date') !=  oldRecord.getFieldValue('custbodycustbody_sched_delivery_date')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbodycustbody_sched_delivery_date')) || !isEmpty(oldRecord.getFieldValue('custbodycustbody_sched_delivery_date')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbodycustbody_sched_delivery_date');
		   if  (isEmpty(nlapiGetFieldValue('custbodycustbody_sched_delivery_date')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Scheduled Delivery Date has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbodycustbody_sched_delivery_date') + "</br>";
           recCarrierMod = true;
       }
      
	   if ( (nlapiGetFieldValue('custbodycustbody_sched_pickup_date') !=  oldRecord.getFieldValue('custbodycustbody_sched_pickup_date')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbodycustbody_sched_pickup_date')) || !isEmpty(oldRecord.getFieldValue('custbodycustbody_sched_pickup_date')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbodycustbody_sched_pickup_date');
		   if  (isEmpty(nlapiGetFieldValue('custbodycustbody_sched_pickup_date')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Scheduled Pickup Date has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbodycustbody_sched_pickup_date') + "</br>";
           recCarrierMod = true;
       }*/
      
	   if ( (nlapiGetFieldValue('custbody_multi_ship_instructions') !=  oldRecord.getFieldValue('custbody_multi_ship_instructions')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbody_multi_ship_instructions')) || !isEmpty(oldRecord.getFieldValue('custbody_multi_ship_instructions')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbody_multi_ship_instructions');
		   if  (isEmpty(nlapiGetFieldValue('custbody_multi_ship_instructions')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Multi Ship Instructions have changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbody_multi_ship_instructions') + "</br>";
           recCarrierMod = true;
       }
      
	   if ( (nlapiGetFieldValue('custbody_order_status') !=  oldRecord.getFieldValue('custbody_order_status')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbody_order_status')) || !isEmpty(oldRecord.getFieldValue('custbody_order_status')) ) &&
          (nlapiGetFieldValue('custbody_order_status') == '3') ) {
		   var oldValue = oldRecord.getFieldText('custbody_order_status');
		   if  (isEmpty(nlapiGetFieldValue('custbody_order_status')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Order Status has changed from: ' + oldValue + ' to: ' + nlapiGetFieldText('custbody_order_status') + "</br>";
           recCarrierMod = true;
       }
      
	   if ( (nlapiGetFieldValue('custbody_temperature_control') !=  oldRecord.getFieldValue('custbody_temperature_control')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbody_temperature_control')) || !isEmpty(oldRecord.getFieldValue('custbody_temperature_control')) ) ) {
		   var oldValue = oldRecord.getFieldValue('custbody_temperature_control');
		   if  (isEmpty(nlapiGetFieldValue('custbody_temperature_control')))
		   {
                var oldValue = 'No Value';
           }				
 		   
           changeCarrierText = changeCarrierText + "<br>" + 'The Temperature Control has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbody_temperature_control') + "</br>";
           recCarrierMod = true;
       }
	   ...
	   if ( (nlapiGetFieldValue('custbody_release_no_donor1') !=  oldRecord.getFieldValue('custbody_release_no_donor1')) && 
	     ( !isEmpty(nlapiGetFieldValue('custbody_release_no_donor1')) || !isEmpty(oldRecord.getFieldValue('custbody_release_no_donor1')) ) ) {
           ...
           changeCarrierText = changeCarrierText + "<br>" + 'The Release Number for Pick-Up location #1 has changed from: ' + oldValue + ' to: ' + nlapiGetFieldValue('custbody_release_no_donor1') + "</br>";
 		   recModified = true;
           recCarrierMod = true;
       }
       ...
       if ( (nlapiGetFieldValue('custbody_warehouse_contact1') !=  oldRecord.getFieldValue('custbody_warehouse_contact1')) && 
           ...
           changeCarrierText = changeCarrierText + "<br>" + 'Warehouse Contact#1 has changed from: ' + oldRecord.getFieldValue('custbody_warehouse1_contact_name') + ' to: ' + nlapiGetFieldValue('custbody_warehouse1_contact_name') + '- Email:' + nlapiGetFieldValue('custbody_warehouse1_contact_email') + '- Phone:' + nlapiGetFieldValue('custbody_warehouse1_contact_phone') + "</br>";
           recModified = true;
 		   recCarrierMod = true;
       }
      ...
      if ( (nlapiGetFieldValue('custbody_drop_off_location_4') !=  oldRecord.getFieldValue('custbody_drop_off_location_4')) && 
       ...
       if ( (nlapiGetFieldValue('custbody_drop_off_shipto_con1') !=  oldRecord.getFieldValue('custbody_drop_off_shipto_con1')) && 
	       ( !isEmpty(nlapiGetFieldValue('custbody_drop_off_shipto_con1')) || !isEmpty(oldRecord.getFieldValue('custbody_drop_off_shipto_con1')) ) ) {
		   var oldValue =  oldRecord.getFieldText('custbody_drop_off_shipto_con1');
		   if  (isEmpty(nlapiGetFieldValue('custbody_drop_off_shipto_con1')))
		   {
                var oldValue = 'No Value';
           }				
 		   
		   changeText = changeText + "<br>" + 'Drop Off Ship To Contact #1 has changed from: ' + oldValue + ' to: ' + nlapiGetFieldText('custbody_drop_off_shipto_con1') + "</br>";
           changeCarrierText = changeCarrierText + "<br>" + 'Drop Off Ship To Contact #1 has changed from: ' + oldRecord.getFieldValue('custbody_dropoff_shipto_contact1_name') + ' to: ' + nlapiGetFieldValue('custbody_dropoff_shipto_contact1_name') + '- Email:' + nlapiGetFieldValue('custbody_dropoff_shipto_contact1_email') + '- Phone:' + nlapiGetFieldValue('custbody_dropoff_shipto_contact1_phone') + "</br>";
           recModified = true;
 		   recCarrierMod = true;
       }
      //Added by Thilaga
...
if (orderType != '1') {
   ...
   if(recCarrierMod){
     nlapiSetFieldValue('custbody_carrier_msg',changeCarrierText);
   }
  ...
  // Now calculate the auction time (Need to add in a friday check and and holiday check
    ...
    if (stType == 'edit' || stType == 'create' || stType == 'approve') {
        ...
        // and the
        orderStatus = nlapiGetFieldValue('custbody_order_status'); // Needs to be Accepted {2} per criteria
        approvalStatus = nlapiGetFieldValue('orderstatus'); // Pending Approval returns 'A' : Pending Fulfillment returns 'B'
        auctionTimeHours = nlapiGetFieldValue('custbody_auction_time_hours') || ''; // Needs to be blank per criteria
       ...
       /* if(stType == 'create' && (orderType == 9 || orderType == 5 || orderType == 3)) {
        ...
        nlapiLogExecution('DEBUG', 'Conditions', 'no tms(F) ' + current_no_tms + ' Order Status(2): ' + orderStatus + ' Approval Status(B): ' + approvalStatus + ' AuctionTime(blank): ' + auctionTimeHours);
      ...
      if ((newRecord.getFieldValue('otherrefnum')!=null)&& (newRecord.getFieldValue('otherrefnum') !=  oldRecord.getFieldValue('otherrefnum') ))
	   {
        ...
        if ((newRecord.getFieldValue('status') == 'Pending Fulfillment' || newRecord.getFieldValue('status') == 'Partially Fulfilled' ||
newRecord.getFieldValue('status') == 'Pending Billing' ||
             newRecord.getFieldValue('status') == 'Pending Billing/Partially Fulfilled' ||
             newRecord.getFieldValue('status') == 'Billed') && (newRecord.getFieldValue('custbody_order_type') == '1' || newRecord.getFieldValue('custbody_order_type') == '2')) {
                ...
                if (magRef != null && magRef != '') {
                  ...
                  if(prodChannel=='1' || prodChannel=='2' || item=='4148'){
                    ...
                    var newLineItemAdded = {
                      ...
                      magRefNum:newRecord.getLineItemValue('item', 'custcol_mag_ref_no', i + 1),
             ...
             newRecord.getFieldValue('status') == 'Pending Billing/Partially Fulfilled') {
                    if (newRecord.getLineItemValue('item', 'quantityfulfilled', i + 1) == 0) {
                      //Modified for Bug 7297 by Thilaga
                        if (parseFloat(oldRecord.getLineItemValue('item', 'quantity', oldLineNum)) != parseFloat(newRecord.getLineItemValue('item', 'quantity', newLineNum))) {
                            changedObj.qtyChange = 'yes';
                            lineQtyChangeDetails = lineQtyChangeDetails + magRefList[i] + ':' + oldRecord.getLineItemValue('item', 'quantity', oldLineNum) + ':' + newRecord.getLineItemValue('item', 'quantity', newLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_vendor_item_no', newLineNum) +
                                ',';
                            recModForSync = true;
                        }
                        if (oldRecord.getLineItemValue('item', 'custcol_pickup_location', oldLineNum) != newRecord.getLineItemValue('item', 'custcol_pickup_location', newLineNum)) {
                            changedObj.qtyChange = 'yes';
                            linePickUpLocChangeDetails = linePickUpLocChangeDetails + magRefList[i] + ':' + oldRecord.getLineItemValue('item', 'custcol_pickup_location', oldLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_pickup_location', newLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_vendor_item_no', newLineNum) +
                                ',';
                            recModForSync = true;
                        }
                        //Modified for Task 7302 by Elizabeth
                        if (oldRecord.getLineItemValue('item', 'custcol_drop_off_location', oldLineNum) != newRecord.getLineItemValue('item', 'custcol_drop_off_location', newLineNum)) {
                            changedObj.qtyChange = 'yes';
                            lineDropOffLocChangeDetails = lineDropOffLocChangeDetails + magRefList[i] + ':' + oldRecord.getLineItemValue('item', 'custcol_drop_off_location', oldLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_drop_off_location', newLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_vendor_item_no', newLineNum) + ':' + newRecord.getLineItemValue('item', 'custcol_member_bank', newLineNum) +
                                ',';
                            recModForSync = true;
                        }
                    }
                ...
                if (newRecord.getFieldValue('status') == 'Billed' || newRecord.getFieldValue('status') == 'Partially Fulfilled') {
                    if (oldRecord.getLineItemValue('item', 'rate', oldLineNum) != newRecord.getLineItemValue('item', 'rate', newLineNum)) {
                        changedObj.qtyChange = 'yes';
                        lineRateChangeDetails = lineRateChangeDetails + magRefList[i] + ':' + oldRecord.getLineItemValue('item', 'rate', oldLineNum) + ':' + newRecord.getLineItemValue('item', 'rate', newLineNum) + ':' + newRecord.getLineItemValue('item', 'rate', newLineNum) +
                            ',';
                        recModForSync = true;
                    }
                }
                if(newRecord.getFieldValue('custbody_order_type')=='2'&& newRecord.getFieldValue('custbody_admin_fees')!=0.00){
      				if ((oldRecord.getLineItemValue('item', 'rate', oldLineNum) != newRecord.getLineItemValue('item', 'rate', newLineNum))||(oldRecord.getLineItemValue('item', 'porate', oldLineNum) != newRecord.getLineItemValue('item', 'porate', newLineNum))){
