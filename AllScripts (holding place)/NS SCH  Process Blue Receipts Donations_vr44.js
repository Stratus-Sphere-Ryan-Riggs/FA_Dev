...
//var savedSearch = 'customsearch_blue_receipt_donations_13';  
		...
		if(resultsSavedSearch && resultsSavedSearch.length > 0)
		{
			for(var i = 0; i < resultsSavedSearch.length; i++) 
			{
				try
				{
					...
					//control Donor
					var memberDonor_check = getEntity('entity', donorNumber, donationID, 'Donor Exception');
					var memberEntityId_check = getEntity('customer', memberNumber, donationID, 'Member Exception');

					var memberDonor = getEntity('entity', donorNumber, donationID, 'Donor Exception');
...
function createOpportunity(donationID, donorID, memberID, items, addressRecReturned) 
{
	try
	{
		...
		//oppRecord.setFieldValue('custbody_donationdate', new Date());
		...
		{
			/*stWarehouseName = donationRecord.getFieldValue('custrecord_warehouse_name');
			stWarehouseAdress = donationRecord.getFieldValue('custrecord_warehouse_address');
			stWarehouseAdress2 = donationRecord.getFieldValue('custrecord_warehouse_address_2');
			stWarehouseCity = donationRecord.getFieldValue('custrecord_warehouse_city');
			stWarehouseState = donationRecord.getFieldValue('custrecord_warehouse_state');
			stWarehouseZip = donationRecord.getFieldValue('custrecord_warehouse_zip');*/
			...
			/*oppRecord.setFieldValue('custbody_warehouse_name', stWarehouseName);
			oppRecord.setFieldValue('custbody_warehouse_address_1', stWarehouseAdress);
			oppRecord.setFieldValue('custbody_warehouse_address_2', stWarehouseAdress2);
			oppRecord.setFieldValue('custbody_warehouse_city', stWarehouseCity);
			oppRecord.setFieldValue('custbody_warehouse_state', stWarehouseState);
			oppRecord.setFieldValue('custbody_warehouse_zip', stWarehouseZip);*/
		}
		else
		{
			//apiLogExecution('debug', 'addressRecReturned type', addressRecReturned.getRecordType());
			//nlapiLogExecution('debug', 'addressRecReturned id', addressRecReturned.getId());
			...
			stWarehouseName = addressRecord.getFieldValue('addressee');
			stWarehouseAdress = addressRecord.getFieldValue('addr1');
			stWarehouseAdress2 = addressRecord.getFieldValue('addr2');
			stWarehouseCity = addressRecord.getFieldValue('city');
			stWarehouseState = addressRecord.getFieldValue('state');
			stWarehouseZip = addressRecord.getFieldValue('zip');*/
		}		
		//Create lines
